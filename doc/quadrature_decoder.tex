...
\node (A) at (0,1.8) {$A$};
\node (B) at (0,-1.8) {$B$};
\node[and gate, draw, inputs=ni] at ($(1.4,-0.4)$) (And1) {};
\node[and gate, draw, inputs=in] at ($(1.4,-1.7)$) (And2) {};
\node[and gate, draw, inputs=nn] at ($(1.4,+1.7)$) (And3) {};
\node[nor gate, draw, inputs=nn] at ($(1.4,+0.4)$) (Nor1) {};
...
\draw (And1.output) -- +(0.2,0) node[above]{II} -- (Nor2.input 1);
\draw (And2.output) -- +(0.2,0) node[above]{III} -- (Nor3.input 2);
\draw (Nor2.output) -| node[branch]{} +(0.1,-0.4) -- +(-1.2,-0.7) |- (Nor3.input 1);
\draw (Nor3.output) -| +(0.1,0.4) -- +(-1.2,0.7) |- (Nor2.input 2);
\draw (Nor2.output) -- (D);
\draw (And3.output) -- +(0.2,0) node[above]{IV} -- (Nor5.input 1);
\draw (Nor1.output) -- +(0.1,0) node[above]{I} -- (Nor4.input 2);
